...
if __name__ == '__main__':
    ...
    q_embedding = [embedding(model,q) for q in question]
    r_embedding = [embedding(model,r) for r in response]
    save_x_embed =[{"a_id":i,"fv":[[x] for x in item]} for i,item in enumerate(r_embedding)]
    save_theta_embed =[{"uid":i,"preference_v":[[x] for x in item]} for i,item in enumerate(q_embedding)]
    with open("Google_llm/arm_info.txt", "w") as f:
        for i in range(len(save_x_embed)):
            f.write(json.dumps(save_x_embed[i]) + '\n')
    with open("Google_llm/user_preference.txt", "w") as f:
